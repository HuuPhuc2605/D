...
btnThem.addActionListener(this);
	...
	private void themNhanVien() {
		try {
			...
			if (!ma.matches("^([A-Z]{3}[0-9]{3})")) {
				JOptionPane.showMessageDialog(this, " Mã nhân viên phải có dạng NVxxx (VD: NV001)");
				return;
			}

			// 3️⃣ Họ và tên chỉ chứa chữ cái
			if (!ho.matches("^([A-ZÀ-Ỹ][a-zà-ỹ]+)(\\s[A-ZÀ-Ỹ][a-zà-ỹ]+){1}$") || !ten.matches("^([A-ZÀ-Ỹ][a-zà-ỹ]+)(\\s[A-ZÀ-Ỹ][a-zà-ỹ]+)*$")) {
				JOptionPane.showMessageDialog(this, " Họ và tên chỉ được chứa chữ cái!");
				return;
			}
			if(!phai.matches("^(?i)Nam|Nu|Nữ|F|M|f|m") ) {
				JOptionPane.showMessageDialog(this, "Phái là Nam hoặc Nữ");
				return;
			}
